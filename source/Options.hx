...
class InstantDeathMode extends Option {
	public override function press():Bool{
		trace("switch");
		FlxG.save.data.instantDeathMode = !FlxG.save.data.instantDeathMode;
		display = updateDisplay();
		return true;
	}

	public override function updateDisplay():String {
		return "Instant Death Mode " + (!FlxG.save.data.instantDeathMode ? "off" : "on");
	}
}
